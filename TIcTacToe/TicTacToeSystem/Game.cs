...
namespace TicTacToeSystem
{
    public class Game : INotifyPropertyChanged
    {
        ...
        public void TakeSpot(int spotnum)
        {
            ...
            if (spot.SpotValue == TurnEnum.None && this.GameStatus == GameStatusEnum.Playing)
            {
                ...
                if (this.GameStatus == GameStatusEnum.Playing)
                {
                    ...
                    if (IsComputerTurn())
                    {
                        //1st step of algorithm - offence
                        DoComputerTurnOffenceDefence(TurnEnum.O);
                        if (IsComputerTurn())
                        {
                            //if turn not taken computer then defence
                            DoComputerTurnOffenceDefence(TurnEnum.X);
                            if (IsComputerTurn())
                            {
        ...
        private void DoComputerTurnOffenceDefence(TurnEnum turn)
        {
